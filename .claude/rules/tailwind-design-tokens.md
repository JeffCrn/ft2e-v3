...
- **Trace de flux** (`TraceFlux.astro`, primitives) : polyligne cuivre verticale dessinee au defilement — la pointe vise le bas du viewport et le rattrape par lissage exponentiel (~0,7 s), ce retard controle rendant le trace visible. Les nœuds carres 7 px se posent EXACTEMENT sur les frontieres entre sections marquees `data-trace-jalon` (mesure DOM + ResizeObserver), le coude de la polyligne s'alignant sur chaque nœud (arbitrages 2026-08-05). Usage : colonne de marge de l'accueil, jalons = cartouche / expertises / secteurs / references / profils. Decoratif, `aria-hidden`, ecrans ≥ 1360 px.
