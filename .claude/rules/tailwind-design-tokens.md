...
- **Appui du hero de l'accueil** (`src/pages/index.astro`, règle `.appui-hero`) : plafonné à **552 px et centré**, comme partout ailleurs. **Le plafond ne connaît pas d'exception d'emploi** — c'était la dernière du site, levée le 2026-08-16 : l'appui de la fiche vedette y était servi à 606 px à 640 de fenêtre (échelle 1,10), 718 à 768 (1,30) et **950 à 1 000 (1,72)**, la grille à 12 colonnes ne le ramenant sous 1,00 qu'au-delà de `lg`. L'argument qui l'avait épargné — « c'est un ornement de couverture, pas une figure de fiche » — ne tient pas : **la règle porte sur le filet, pas sur le rôle du dessin.** La lisibilité, elle, *gagnait* à l'étirement (le mono de 10 px montait à 17) ; ce qui se dégradait est l'épaisseur des filets de 1 px, or la charte fait porter le rang d'un filet par son **opacité**, ce qui suppose que son épaisseur ne bouge pas. Un dessin agrandi rend donc tous ses rangs faux à la fois. Corollaire d'implantation, identique à celui de la vignette : le plafond enveloppe **le dessin et sa légende** (ici la carte-lien, qui faisait la même largeur au pixel), il est écrit **chez qui compose le média** et jamais dans `Hero.astro`, et il s'écrit **en CSS de composant** — une classe `max-w-[…]` en valeur arbitraire, unique dans le dépôt, disparaîtrait sans un mot du build (incident du 2026-08-08).
