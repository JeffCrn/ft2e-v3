...
**Implantation de la légende et des équerres (§ 13)** — la charte veut la légende **en bas à gauche** *et* les équerres intactes (« repère de tirage, jamais un encadrement ») : les deux ne peuvent pas se disputer l'angle. La géométrie tranche, et elle est **dérivée**, jamais réglée à l'œil : les jetons `--equerre-cote` (18 px) et `--equerre-retrait` (5 px) donnent `--equerre-gouttiere` = 28 px, dont découlent `.legende-media` (`left: gouttière`, `bottom: retrait`, `max-width: 100% − 2 × gouttière`) et les quatre équerres de `CoinsCuivre`. La légende partage donc la ligne de pied des équerres sans jamais empiéter dessus, quelle que soit sa longueur — elle déborde vers le haut, où aucune équerre ne l'attend. **Ne pas repositionner une légende par des offsets en dur** (`left-4`, `bottom-3`, `left-[12.5%]`) : le lien avec les équerres serait rompu au premier changement de mesure.
