---
titre: "Électricité CFO/CFA"
accroche: "Courants forts, courants faibles, photovoltaïque, IRVE, gestion technique du bâtiment. Conception conforme NF C 15-100."
icone: "electricite"
ordre: 4
livrables:
  - "Bilan de puissance et dimensionnement"
  - "Plans et schémas unifilaires"
  - "CCTP lots CFO et CFA"
  - "Dimensionnement IRVE (bornes de recharge)"
  - "Cahier GTB (gestion technique du bâtiment)"
  - "Analyse des offres et visa d’exécution"
typique_pour: [Logements, "Tertiaire / ERP", "Industriel et commercial", Monotechnique]
missions_liees: ["Électricité CFO", "Électricité CFA"]
faq:
  - question: "Qu’est-ce que la GTB et pourquoi la prévoir dès la conception ?"
    reponse: "La GTB (gestion technique du bâtiment) pilote et supervise les équipements techniques (CVC, éclairage, stores). Intégrée dès la conception, elle permet d’optimiser les consommations et de répondre aux exigences du décret tertiaire."
  - question: "Les bornes IRVE sont-elles obligatoires ?"
    reponse: "Oui, pour les bâtiments neufs ou rénovés disposant d’un parc de stationnement. La loi LOM impose un pré-câblage et un nombre minimal de bornes selon l’usage du bâtiment."
---

## Courants forts, courants faibles : deux disciplines, une coordination

L’électricité d’un bâtiment se structure en deux familles complémentaires. Les courants forts (CFO) couvrent l’alimentation générale, la distribution, l’éclairage et les prises de courant. Les courants faibles (CFA) regroupent les réseaux de communication, le contrôle d’accès, la vidéosurveillance, la sonorisation et la détection incendie — FT2E a notamment conçu l’installation de vidéosurveillance du centre hospitalier Marius Lacroix à La Rochelle, pour le Groupe hospitalier La Rochelle-Ré-Aunis. FT2E conçoit les deux lots de manière intégrée, en garantissant la conformité à la norme NF C 15-100 et la cohérence des chemins de câbles, des locaux techniques et des tableaux de distribution.

## Du bilan de puissance au schéma unifilaire

Toute étude électrique commence par le bilan de puissance : inventaire des récepteurs, simultanéité d’usage, puissance souscrite. Ce bilan conditionne le dimensionnement du poste de transformation, du TGBT et des tableaux divisionnaires. Aux dix logements du [Pas des Bœufs, au Bois-Plage-en-Ré](/references/logements-pas-des-boeufs-bois-plage), les 90 kVA installés se ramènent à 71 kVA utiles après foisonnement, recharge des véhicules comprise. FT2E établit également les pré-dossiers de raccordement ENEDIS et, sur les [sites industriels](/secteurs/industriel-commercial), dimensionne la distribution HTA/BT et les installations de protection contre la foudre à partir de l’analyse du risque foudre (ARF). FT2E produit les schémas unifilaires, les plans de distribution et les spécifications détaillées au CCTP. Chaque choix — calibre de protection, section de câble, type de chemin de câbles — est tracé et justifié. Le dossier est conçu pour être directement exploitable par les entreprises en phase d’appel d’offres.

## IRVE : anticiper la mobilité électrique

La loi LOM (loi d’orientation des mobilités) impose un pré-équipement en bornes de recharge pour véhicules électriques (IRVE) dans les bâtiments neufs et rénovés disposant de places de stationnement. FT2E dimensionne le pré-câblage (fourreaux, câbles, protections) et les bornes elles-mêmes en fonction du programme : résidentiel collectif, tertiaire, ERP. Sur l’[habitat inclusif de Salignac-sur-Charente](/references/habitat-inclusif-salignac-sur-charente), dont le parc de stationnement entre dans la tranche de dix à vingt places, des fourreaux desservent 100 % des places et 22 kVA sont réservés au local technique pour l’infrastructure de recharge. Dans la même logique d’électrification des usages, FT2E conçoit également les installations photovoltaïques associées aux bâtiments tertiaires et industriels, du dimensionnement au raccordement. À l’[ancien siège communautaire de Marennes](/references/ancien-siege-communautaire-marennes), 108 modules totalisent 33,48 kWc en toiture, raccordés en revente totale.

## GTB : piloter et optimiser en exploitation

La gestion technique du bâtiment (GTB) est devenue un levier majeur de performance énergétique. En supervisant le CVC, l’éclairage, les stores et les équipements techniques depuis un poste central, la GTB permet de réduire les consommations de 15 à 25 % par le seul ajustement des plages horaires et des consignes. FT2E rédige le cahier GTB dès la phase de conception, définit les points de comptage, les scénarios de régulation et les interfaces avec les lots CVC et éclairage. Cette anticipation évite les reprises coûteuses en phase chantier et garantit un système opérationnel dès la livraison.

## Une présence de la conception à la réception

En mission complète, FT2E analyse les offres des entreprises électriques, vise les plans d’exécution et assure le suivi de chantier jusqu’à la mise sous tension et aux essais fonctionnels. Cette continuité entre conception et réalisation est particulièrement précieuse sur les lots électriques, où les interfaces entre CFO, CFA, [SSI](/secteurs/coordination-ssi) et GTB sont nombreuses et exigent une coordination rigoureuse. Sur la [passerelle cyclo-piétonne du Carreau d’Or, à Marans](/references/passerelle-ecluse-carreau-d-or-marans), l’électricité est le seul lot technique de l’ouvrage — l’éclairage y passe en très basse tension, logé dans l’engravure des mains courantes.
