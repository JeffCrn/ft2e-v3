...
Trois mécanismes à ce jour, dispatchés sur le bloc de l'extraction.
...
`relais` — troisième emploi, mairie des Portes-en-Ré (2026-08-27) : **le
service reste, le chantier se déplace.** Un axe des temps et trois
registres — une ligne encrée continue (le service public, jamais
interrompu), deux blocs de chantier successifs au-dessous d'elle, et la
marche orthogonale qui les relie : le premier chantier libère l'emprise du
second. Texte masqué, il reste une ligne ininterrompue et deux blocs
décalés reliés par une marche — la continuité contre le déplacement.
"""
...
from _tronc import (W, H, MARGE, UTILE, VW, VH, V_MARGE, AW, AH, A_MARGE,
                    ...
                    polyligne, fleche, entete_style, racine_appui,
...
# ─────────────────────────────────────────────────────────────────────────────
# Mécanisme `relais` — un service continu, des chantiers qui se déplacent
# Troisième emploi, mairie des Portes-en-Ré (2026-08-27) : réhabiliter en site
# occupé. Un axe des temps et trois registres : une ligne encrée continue (le
# service public, jamais interrompu), deux blocs de chantier qui se succèdent
# au-dessous d'elle, et le relais orthogonal qui les relie — le premier
# chantier libère l'emprise du second. Texte masqué, il reste une ligne
# ininterrompue et deux blocs décalés reliés par une marche : la continuité
# d'un côté, le déplacement de l'autre. Les repères sont nommés par mécanisme
# (préfixe RL_) — deux affectations d'un même nom au niveau du module se
# marchent dessus, et c'est le PREMIER dessin qui se recompose faux.
# ─────────────────────────────────────────────────────────────────────────────

RL_X0 = MARGE                  # l'axe des temps occupe la largeur utile
RL_X1 = W - MARGE              # 1144
RL_Y_SERV_LIBELLE = 240
RL_Y_SERV = 262
RL_Y_SERV_DETAIL = 286
RL_Y_OT = 330
RL_H_BLOC = 56
RL_Y_MAIRIE = 440
RL_Y_AXE = 600
...
def composer_relais(donnees):
    rl = donnees["relais"]
    axe = rl["axe"]
    ech = _echelle(axe, RL_X0, RL_X1)
    out = []
    A = out.append
    depassements = []
    ...
    # ── En-tête du registre + double légende (le signe toujours doublé) ──────
    l_ch = mesurer(rl["legende_chantier"], 10, "mono", 10 * 0.14)
    x_chip = RL_X1 - l_ch - 22
    A(rect_bord(x_chip, Y_ENTETE_A - 8, 14, 8, "calcaire", "filet-1"))
    A(texte(RL_X1, Y_ENTETE_A, rl["legende_chantier"], "mono", 10, 500,
            "pivot", ancre="end", tracking=10 * 0.14))
    x_fin_serv = x_chip - 24
    l_serv = mesurer(rl["legende_service"], 10, "mono", 10 * 0.14)
    A(ligne(x_fin_serv - l_serv - 24, Y_ENTETE_A - 3.5,
            x_fin_serv - l_serv - 8, Y_ENTETE_A - 3.5, "encre", 2.5))
    A(texte(x_fin_serv, Y_ENTETE_A, rl["legende_service"], "mono", 10, 500,
            "pivot", ancre="end", tracking=10 * 0.14))
    controler("en-tête relais", rl["entete"], 10, "mono",
              x_fin_serv - l_serv - 24 - 24 - MARGE, 10 * 0.14)
    A(texte(MARGE, Y_ENTETE_A, rl["entete"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))

    # ── La grille des années — sous les registres, jamais au travers ─────────
    annees_x = [ech(b.isoformat()) for b in _annees_bornes(axe)]
    for ax_ in annees_x:
        A(ligne(ax_, RL_Y_SERV_LIBELLE - 18, ax_, RL_Y_AXE, "filet-3", 1.0))

    # ── Registre 1 — le service public, une ligne qui ne s'arrête pas ────────
    srv = rl["service"]
    controler("libellé service", srv["libelle"], 15, "sans-600", 400)
    A(texte(MARGE, RL_Y_SERV_LIBELLE, srv["libelle"], "sans", 15, 600,
            "encre", wdth=112))
    controler("mention service", srv["mention"], 10, "mono", 300, 10 * 0.14)
    A(texte(RL_X1, RL_Y_SERV_LIBELLE, srv["mention"], "mono", 10, 500,
            "pivot", ancre="end", tracking=10 * 0.14))
    A(ligne(RL_X0, RL_Y_SERV, RL_X1, RL_Y_SERV, "encre", 2.5))
    _marque(A, RL_X0 + COTE_MARQUE / 2, RL_Y_SERV)
    _marque(A, RL_X1 - COTE_MARQUE / 2, RL_Y_SERV)
    controler("détail service", srv["detail"], 10, "mono", 500, 10 * 0.14)
    A(texte(MARGE, RL_Y_SERV_DETAIL, srv["detail"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))

    # ── Registre 2 — l'office de tourisme, un bloc étroit en tête d'axe ──────
    ot, mairie = rl["chantiers"][0], rl["chantiers"][1]
    x_ot0, x_ot1 = ech(ot["debut"]), ech(ot["fin"])
    cy_ot = RL_Y_OT + RL_H_BLOC / 2
    A(rect_bord(x_ot0, RL_Y_OT, x_ot1 - x_ot0, RL_H_BLOC, "calcaire", "filet-1"))
    A(texte(x_ot0 + PAD, cy_ot + 3.5, ot["mention"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))
    controler("mention OT", ot["mention"], 10, "mono",
              x_ot1 - x_ot0 - 2 * PAD, 10 * 0.14)
    A(texte(x_ot1 + PAD, RL_Y_OT + 23, ot["libelle"], "sans", 15, 600,
            "encre", wdth=112))
    l_lib_ot = mesurer(ot["libelle"], 15, "sans-600")
    controler("détail OT", ot["detail"], 10, "mono",
              RL_X1 - (x_ot1 + PAD), 10 * 0.14)
    A(texte(x_ot1 + PAD, RL_Y_OT + 41, ot["detail"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))

    # ── Le relais : la marche orthogonale du premier chantier au second ──────
    x_ma0, x_ma1 = ech(mairie["debut"]), ech(mairie["fin"])
    cy_ma = RL_Y_MAIRIE + RL_H_BLOC / 2
    A(polyligne([(x_ot1, cy_ot), (x_ma0, cy_ot), (x_ma0, RL_Y_MAIRIE - 7)],
                "encre", 1.5))
    A(fleche(x_ma0, RL_Y_MAIRIE - 2, "encre", direction="bas", taille=8))
    # Le libellé du relais se pose À DROITE de la descente, dans le vide entre
    # les deux registres — centré sur la marche, il percutait le détail du bloc
    # OT (vu au contrôle à 1152 px).
    controler("libellé relais", rl["relais"]["libelle"], 10, "mono",
              RL_X1 - (x_ma0 + 12), 10 * 0.14)
    A(texte(x_ma0 + 12, (cy_ot + RL_Y_MAIRIE) / 2 + 10, rl["relais"]["libelle"],
            "mono", 10, 500, "pivot", tracking=10 * 0.14))

    # ── Registre 3 — la mairie et la salle des fêtes ─────────────────────────
    A(rect_bord(x_ma0, RL_Y_MAIRIE, x_ma1 - x_ma0, RL_H_BLOC,
                "calcaire", "filet-1"))
    controler("libellé mairie", mairie["libelle"], 15, "sans-600",
              x_ma1 - x_ma0 - 2 * PAD)
    A(texte(x_ma0 + PAD, RL_Y_MAIRIE + 23, mairie["libelle"], "sans", 15, 600,
            "encre", wdth=112))
    controler("détail mairie", mairie["detail"], 10, "mono",
              x_ma1 - x_ma0 - 2 * PAD, 10 * 0.14)
    A(texte(x_ma0 + PAD, RL_Y_MAIRIE + 41, mairie["detail"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))
    l_mention_ma = mesurer(mairie["mention"], 10, "mono", 10 * 0.14)
    controler("mention mairie", mairie["mention"], 10, "mono",
              x_ma1 - x_ma0 - 2 * PAD
              - mesurer(mairie["libelle"], 15, "sans-600") - 20, 10 * 0.14)
    A(texte(x_ma1 - PAD, RL_Y_MAIRIE + 23, mairie["mention"], "mono", 10, 500,
            "pivot", ancre="end", tracking=10 * 0.14))

    # ── La livraison : une descente vers l'axe, la marque du terme ───────────
    lv = rl["livraison"]
    x_lv = ech(lv["date"])
    A(ligne(x_lv, RL_Y_MAIRIE + RL_H_BLOC, x_lv, RL_Y_AXE, "encre", 1.5))
    controler("libellé livraison", lv["libelle"], 10, "mono",
              x_lv - 8 - MARGE, 10 * 0.14)
    A(texte(x_lv - 8, (RL_Y_MAIRIE + RL_H_BLOC + RL_Y_AXE) / 2 + 4,
            lv["libelle"], "mono", 10, 500, "pivot", ancre="end",
            tracking=10 * 0.14))

    # ── L'axe des temps, gradué aux millésimes ───────────────────────────────
    A(ligne(RL_X0, RL_Y_AXE, RL_X1, RL_Y_AXE, "encre", 1.5))
    bornes = [RL_X0] + annees_x + [RL_X1]
    for b in bornes:
        A(ligne(b, RL_Y_AXE, b, RL_Y_AXE + 6, "encre", 1.0))
    for (a_, b_), an in zip(zip(bornes, bornes[1:]), axe["annees"]):
        A(texte((a_ + b_) / 2, RL_Y_ANNEES, an, "mono", 10, 500, "pivot",
                ancre="middle", tracking=10 * 0.14))
    ...
    succession = x_ot1 < x_ma0
    controles = {
        "gabarit": f"{W} x {H} — rapport {W/H:.4f} (3:2 exact)",
        "demonstration": f"une ligne continue de x {RL_X0} à {RL_X1} (le service, "
                         f"sans coupure) contre deux blocs disjoints "
                         f"{'strictement successifs' if succession else '⚠ QUI SE CHEVAUCHENT'} "
                         f"(OT {x_ot0:.0f}–{x_ot1:.0f}, mairie {x_ma0:.0f}–"
                         f"{x_ma1:.0f}) reliés par une marche orthogonale — la "
                         f"géométrie porte la coexistence, aucun chiffre de la "
                         f"fiche n’est répété",
        "topologie": f"axe des temps x {RL_X0}–{RL_X1} ; service y {RL_Y_SERV}, "
                     f"bloc OT {RL_Y_OT}–{RL_Y_OT + RL_H_BLOC}, bloc mairie "
                     f"{RL_Y_MAIRIE}–{RL_Y_MAIRIE + RL_H_BLOC}, axe à y {RL_Y_AXE}",
        "bas_du_dessin": f"axe à {RL_Y_AXE}, millésimes à {RL_Y_ANNEES}, phrase "
                         f"à {Y_PHRASE}, cartouche {Y_CARTOUCHE}–"
                         f"{Y_CARTOUCHE + H_CARTOUCHE}, marge basse "
                         f"{H - (Y_CARTOUCHE + H_CARTOUCHE)} px",
        "reserve_profonde": f"cartouche {largeur} x {H_CARTOUCHE} px = "
...
def composer_vignette_relais(donnees):
    """La vignette : le motif seul — la ligne continue, les deux blocs
    décalés, la marche du relais. Détails, mentions et millésimes d'axe sont
    laissés à la planche ; deux étiquettes et deux valeurs se lisent."""
    rl = donnees["relais"]
    axe = rl["axe"]
    x0, x1 = V_MARGE, VW - V_MARGE
    ech = _echelle(axe, x0, x1)
    y_serv, y_ot, y_ma, h_bloc = 64, 88, 128, 24
    bas = 176
    ...
    for b in _annees_bornes(axe):
        bx = ech(b.isoformat())
        A(ligne(bx, y_serv - 18, bx, bas, "filet-3", 1.0))

    A(texte(x1, y_serv - 8, rl["service"]["mention"], "mono", 10, 500,
            "pivot", ancre="end", tracking=10 * 0.14))
    A(ligne(x0, y_serv, x1, y_serv, "encre", 1.6))
    A(rect(x0, y_serv - 2, 4, 4, "encre"))
    A(rect(x1 - 4, y_serv - 2, 4, 4, "encre"))

    ot, mairie = rl["chantiers"][0], rl["chantiers"][1]
    x_ot0, x_ot1 = ech(ot["debut"]), ech(ot["fin"])
    x_ma0, x_ma1 = ech(mairie["debut"]), ech(mairie["fin"])
    cy_ot, cy_ma = y_ot + h_bloc / 2, y_ma + h_bloc / 2
    A(rect_bord(x_ot0, y_ot, x_ot1 - x_ot0, h_bloc, "calcaire", "filet-2"))
    A(rect_bord(x_ma0, y_ma, x_ma1 - x_ma0, h_bloc, "calcaire", "filet-2"))
    A(polyligne([(x_ot1, cy_ot), (x_ma0, cy_ot), (x_ma0, y_ma - 5)],
                "encre", 1.2))
    A(fleche(x_ma0, y_ma - 1, "encre", direction="bas", taille=6))

    # L'étiquette du bloc OT se pose AU-DESSUS de lui : à sa droite, elle
    # était barrée par la marche du relais (vu au contrôle à 274 px).
    A(texte(x_ot0, y_ot - 7, ot["libelle"], "sans", 12, 600,
            "encre", wdth=112))
    A(texte(x_ot0 + 4, cy_ot + 3.5, "2023", "mono", 10, 500,
            "pivot", tracking=10 * 0.14))
    A(texte(x_ma0 + 8, cy_ma + 4, "Mairie", "sans", 12, 600,
            "encre", wdth=112))
    A(texte(x_ma0, y_ma + h_bloc + 14, "2024 → 2025", "mono", 10, 500,
            "pivot", tracking=10 * 0.14))
    ...
    controles = {
        ...
        "echelle_de_rendu": f"carte de projet mesurée de 274 à 296 px — "
                            f"échelle {274/VW:.2f} à {296/VW:.2f}",
        "corps_minimal": f"9 px dans le repère — rendu à {9*274/VW:.1f} px au pire cas",
        "motif": f"le motif seul : la ligne continue x {x0:.0f}–{x1:.0f}, deux "
                 f"blocs décalés (OT {x_ot0:.0f}–{x_ot1:.0f}, mairie "
                 f"{x_ma0:.0f}–{x_ma1:.0f}), la marche du relais — deux "
                 f"étiquettes et deux valeurs ; détails et axe laissés à la planche",
        "bas_du_dessin": f"{bas:.0f} px, marge basse {VH - bas:.0f} px",
    }
    return "\n".join(out) + "\n", controles


def composer_appui_relais(donnees):
    """L'appui du hero : le motif entier à l'échelle 1 — la ligne du service,
    les deux blocs, la marche, la livraison et l'axe des millésimes. Densité
    intermédiaire : trois nœuds datés, la double légende courte — sans phrase
    de principe ni cartouche."""
    rl = donnees["relais"]
    axe = rl["axe"]
    x0, x1 = A_MARGE, AW - A_MARGE
    ech = _echelle(axe, x0, x1)
    y_serv_lbl, y_serv = 78, 96
    y_ot, y_ma, h_bloc = 130, 190, 36
    y_detail_ma = 240
    y_livraison = 264
    y_axe = 300
    y_annees = 318
    ...
    leg_ch = rl.get("legende_chantier_court", rl["legende_chantier"])
    leg_srv = rl.get("legende_service_court", rl["legende_service"])
    l_ch = mesurer(leg_ch, 10, "mono", 10 * 0.14)
    x_chip = x1 - l_ch - 20
    A(rect_bord(x_chip, 27, 14, 8, "calcaire", "filet-1"))
    A(texte(x1, 34, leg_ch, "mono", 10, 500, "pivot",
            ancre="end", tracking=10 * 0.14))
    x_fin_srv = x_chip - 20
    l_srv = mesurer(leg_srv, 10, "mono", 10 * 0.14)
    A(ligne(x_fin_srv - l_srv - 20, 30.5, x_fin_srv - l_srv - 6, 30.5,
            "encre", 2.0))
    A(texte(x_fin_srv, 34, leg_srv, "mono", 10, 500, "pivot",
            ancre="end", tracking=10 * 0.14))

    for b in _annees_bornes(axe):
        bx = ech(b.isoformat())
        A(ligne(bx, y_serv_lbl - 14, bx, y_axe, "filet-3", 1.0))

    srv = rl["service"]
    A(texte(x0, y_serv_lbl, srv["libelle"], "sans", 13, 600, "encre", wdth=112))
    A(texte(x1, y_serv_lbl, srv["mention"], "mono", 10, 500, "pivot",
            ancre="end", tracking=10 * 0.14))
    A(ligne(x0, y_serv, x1, y_serv, "encre", 2.0))
    A(rect(x0, y_serv - 2.5, 5, 5, "encre"))
    A(rect(x1 - 5, y_serv - 2.5, 5, 5, "encre"))

    ot, mairie = rl["chantiers"][0], rl["chantiers"][1]
    x_ot0, x_ot1 = ech(ot["debut"]), ech(ot["fin"])
    x_ma0, x_ma1 = ech(mairie["debut"]), ech(mairie["fin"])
    cy_ot, cy_ma = y_ot + h_bloc / 2, y_ma + h_bloc / 2
    A(rect_bord(x_ot0, y_ot, x_ot1 - x_ot0, h_bloc, "calcaire", "filet-1"))
    A(texte(x_ot1 + 10, cy_ot - 2, ot["libelle"], "sans", 13, 600,
            "encre", wdth=112))
    A(texte(x_ot1 + 10, cy_ot + 14, ot["mention"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))

    # La marche part du BAS du bloc OT et passe par un couloir entre les deux
    # registres : à hauteur du bloc, elle traversait ses étiquettes (vu au
    # contrôle à 552 px).
    x_c_ot = (x_ot0 + x_ot1) / 2
    y_corridor = (y_ot + h_bloc + y_ma) / 2
    A(polyligne([(x_c_ot, y_ot + h_bloc), (x_c_ot, y_corridor),
                 (x_ma0, y_corridor), (x_ma0, y_ma - 6)], "encre", 1.5))
    A(fleche(x_ma0, y_ma - 1, "encre", direction="bas", taille=7))

    A(rect_bord(x_ma0, y_ma, x_ma1 - x_ma0, h_bloc, "calcaire", "filet-1"))
    A(texte(x_ma0 + 10, cy_ma + 4, mairie["libelle"], "sans", 13, 600,
            "encre", wdth=112))
    A(texte(x_ma0, y_detail_ma, mairie["detail"], "mono", 10, 500,
            "pivot", tracking=10 * 0.14))

    lv = rl["livraison"]
    x_lv = ech(lv["date"])
    A(ligne(x_lv, y_ma + h_bloc, x_lv, y_axe, "encre", 1.5))
    A(texte(x_lv - 6, y_livraison, lv["libelle"], "mono", 10, 500,
            "pivot", ancre="end", tracking=10 * 0.14))
    ...
    for b in bornes:
        A(ligne(b, y_axe, b, y_axe + 5, "encre", 1.0))
    for (a_, b_), an in zip(zip(bornes, bornes[1:]), axe["annees"]):
        A(texte((a_ + b_) / 2, y_annees, an, "mono", 10, 500, "pivot",
                ancre="middle", tracking=10 * 0.14))

    A("</svg>")
    return "\n".join(out) + "\n", controles_appui(
        motif=f"le motif entier : la ligne continue du service x {x0}–{x1}, "
              f"deux blocs successifs (OT {x_ot0:.0f}–{x_ot1:.0f}, mairie "
              f"{x_ma0:.0f}–{x_ma1:.0f}), la marche du relais, la descente de "
              f"livraison et l’axe des millésimes — trois nœuds datés "
              f"(PRINTEMPS 2023, 55 CR, LIVRAISON), double légende courte ; "
              f"phrase et cartouche laissés à la planche",
        bas=f"axe à {y_axe} px, millésimes à {y_annees} px, marge basse "
            f"{AH - y_annees} px")


def _composer(donnees):
    if "relais" in donnees:
        return composer_relais(donnees)
...
def _composer_vignette(donnees):
    if "relais" in donnees:
        return composer_vignette_relais(donnees)
    if "divergence" in donnees:
...
def _composer_appui(donnees):
    if "relais" in donnees:
        return composer_appui_relais(donnees)
    if "divergence" in donnees:
