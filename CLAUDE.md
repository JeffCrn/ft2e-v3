...
2. Présente des **fiches projets réelles**, sourcées pièce par pièce sur les dossiers d'affaires FT2E — **quarante-sept au 2026-09-03, et le chantier est CLOS à ce compte** (l'objectif de cinquante a été revu par l'utilisateur le 2026-09-03 : `2020.zip` n'existe pas, deux affaires du classeur sont hors d'atteinte, et le classeur lui-même n'a jamais mené qu'à quarante-neuf). ⚠ **Ce compte ne se met pas à jour tout seul et il a menti treize sessions durant** (il annonçait vingt-trois jusqu'au 2026-09-01, alors que le chantier des 27 nouvelles fiches en avait déjà versé quatorze) : **le compte qui fait foi est la MESURE DIRECTE, et elle seule** : `ls src/content/projets/*.md | wc -l`. ⚠⚠ La règle antérieure — « le nombre de lignes du § Suivi du plan, plus vingt-trois » — **était fausse et a été retirée le 2026-09-03** : elle encodait une hypothèse tacite, *une session = une fiche*, vraie de N01 à N24 et fausse dès la N25, qui est une session de clôture ne produisant aucune fiche. Elle annonçait alors 48 pour 47. C'est le défaut même qu'elle avait été écrite pour réparer : **un compte dérivé ne sait pas quand sa propre prémisse cesse de tenir**, et le corriger en ajustant la constante n'aurait fait que déplacer le mensonge d'un cran. **Plus aucune fiche de démonstration : les huit `demo: true` ont été supprimées le 2026-08-08 à la demande de FT2E.**
...
- ✅ **Le monogramme a été repris — demande client du 2026-09-06, arbitrée et posée le 2026-09-11.** Deux changements : l’approche du mot F T 2 E resserrée de **11 à 8 unités** d’écart (`translate` 32 / 66 / 98, mot à **122,0** unités au lieu de 131,0), et la signature passée de « BUREAU D’ÉTUDES TECHNIQUES » à « **BET FLUIDES / ET THERMIQUE** », **deux lignes justifiées sur l’empan du mot**. ⚠ `Logo.astro` était la transcription EXACTE du dessin de la charte : ce sont donc des **amendements A15 et A16**, consignés dans `.claude/rules/tailwind-design-tokens.md` — **ne pas les « corriger » vers le PDF**. ⚠ **Justifier en chasse fixe impose un interlettrage PAR LIGNE** (6,500 et 5,536) : `s = (W − encre(0)) / (n − 1)`. Changer un mot de la signature oblige à **refaire ce calcul**, jamais à ajuster à l’œil. ⚠ Et l’encre **ne se mesure pas avec `getBBox()`**, qui rend la boîte d’approche, espace traînant compris — les largeurs publiées avant le 2026-09-11 (200,2 ; 107,8) sont des largeurs d’approche. 🟠 **Trois réserves restent ouvertes** : le libellé est à **confirmer** (le texte de FT2E disait « BUREAU », son image annotée montrait « BET ») ; l’**arbitrage de portée n’a pas été rendu**, donc l’`aria-label` du monogramme et `SITE_TAGLINE` disent encore « bureau d’études techniques » — et le SVG portant `role="img"`, un lecteur d’écran n’entend **que** l’attribut, jamais le texte dessiné ; enfin la signature **annonce moins que le site ne montre** (électricité, SSI, BIM), et c’est la couche description qui porte désormais le périmètre complet. Bilan, recette et prompt de suite : `docs/superpowers/plans/2026-09-06-chantier-monogramme.md` § 8.
- ✅ **Les six articles SEO de lancement sont écrits** — deux le 2026-09-04 en session N26 (RE2020 en logement, coordination SSI en cinquième catégorie) et quatre le même jour en N27 (PAC aérothermique ou géothermique, études d’exécution, décret tertiaire 2030, IRVE en logement collectif neuf). C’était le critère de sortie de Phase 4 qui ne dépendait de personne d’autre ; **il ne reste à la Phase 4 que Decap, qui est hors dépôt.** Le maillage du cocon sémantique EXISTE depuis la N26 : un article déclare ses `piliers` une seule fois au frontmatter, les onze pages piliers ramassent — **ne pas ajouter de champ symétrique `articles_lies`**, ce dessin a été écarté le 2026-09-04. ⚠ **Le cocon plafonne à 8 piliers sur 11, et c’est définitif** : trois pages n’ont aucun satellite et aucune n’atteint les 3 à 5 que `seo-geo.md` demande. Six articles ne couvrent pas onze piliers — c’est structurel, à dire à FT2E (périmètre contractuel à six, ou devis) plutôt qu’à laisser découvrir. ⚠ `docs/17` dit aussi les six sujets « à valider en cadrage » : les avoir écrits ne les valide pas.
- **Pas encore migré, et la cible de production est OVH** — pas Vercel. Le déploiement de démonstration reste `ft2e-v3.vercel.app` ; la mise en production se fera sur **OVHcloud Webhosting Pro** (`docs/09-deploiement-ovh.md`), à l’adresse `ft2e.fr`. ⚠ **Deux conséquences connues, à ne pas découvrir à la bascule.** (a) Le proxy OAuth de Decap — `api/auth.js` et `api/callback.js` — est écrit **au format fonction Vercel** : un mutualisé OVH ne l’exécutera pas tel quel. Deux chemins à arbitrer : le réécrire en PHP (OVH expose PHP 8.x), ou **garder le proxy sur Vercel** pendant qu’OVH sert le site (Decap admet un `base_url` distinct). **Réparer l’OAuth aujourd’hui ne survit donc pas nécessairement à la bascule** : le blocage de rang A et la migration sont couplés. (b) Le `X-Robots-Tag` passe alors par le `.htaccess` que `docs/09` prévoit, et non plus par `vercel.json` — `docs/19-migration-production.md` est **à réconcilier avec `docs/09`**.
...
- Photos équipe (collective + 7 portraits individuels) sont des **images de démonstration générées par IA** marquées `DÉMO` ; reportage photographique professionnel prévu en phase de production. Les visuels de secteurs, eux, sont depuis le 2026-08-25 **44 photographies réelles (© FT2E)** versées par le chantier du bloc secteurs — 17 fichiers fournis écartés (14 perspectives ou photomontages de tiers, 3 documents sur fond de plan d'architecte, au titre du précédent des planches), et des **artefacts d'agrandissement génératif** relevés sur plusieurs clichés retenus (petits textes nets mais faux), **à faire valider par FT2E avant production** (§ 5 du plan du chantier). Les visuels des treize fiches réelles, eux, sont authentiques (perspectives d'architecte, vues de dossier, extraits de nos propres plans).
- **Marqueurs `[DÉMO]` restants : 0 depuis le 2026-08-25** — les sept `image_alt` des secteurs, derniers du site, sont tombés avec les images IA qu'ils signalaient, remplacées par le corpus de clichés réels du bloc secteurs (arbitrage E). ⚠ Le compte était de 8 jusqu'au 2026-08-16, mais le huitième — celui de l'unique actualité — **ne pouvait jamais s'afficher** : `image` et `image_alt` étaient déclarés au Zod et à Decap, lus par aucun rendu, et pointaient un fichier inexistant dans un répertoire vide. Les champs ont été supprimés, et le compte avec eux. Un marqueur qui ne peut pas paraître n'est pas un marqueur en attente de reportage : c'est un champ mort qui gonflait le décompte. Ils marquaient des visuels de démonstration générés par IA et devaient se lever au reportage photographique — les sept derniers se sont finalement levés par la fourniture de clichés réels (arbitrage E du 2026-08-25). Les **10 marqueurs en prose ont été levés le 2026-08-09** : chaque exemple fictif a été remplacé par un exemple relevé sur une des 19 fiches réelles, avec lien interne vivant — au passage, les six liens `/references/…` de `src/content/` étaient **tous morts** depuis la suppression des fiches de démonstration.
...
- **Monogramme** (`Logo.astro`) : cadre ouvert + flux débordant + lettres FT2E + signature mono. **Repris le 2026-09-11 sur demande FT2E** — approche du mot resserrée de 11 à 8 unités d’écart (amendement **A15**) et signature passée de « BUREAU D’ÉTUDES TECHNIQUES » à « BET FLUIDES / ET THERMIQUE », **deux lignes justifiées sur l’empan du mot** (amendement **A16**). Le verrouillage reste 330 × 90. Versions `principal`/`inverse`/`valeur-unique`, forme `cadre` sous 180 px, hauteur min 28 px. Le débord ne se recadre jamais ; ni ombre ni contour.
...
- **Index des références** (`/references`) : **grille de cartes** `CarteProjet` depuis le 2026-08-15 (**amendement A9** — la charte prescrivait l'inverse, pour une vignette qui n'existe plus), servie **sur trois colonnes à droite d'un rail de filtres** depuis le 2026-08-26 (demande FT2E, les vignettes de filtre de 130 px étant trop petites) : au-dessus de 1 024 px les filtres occupent la première colonne du rythme (216 px à 1 024, 276 px dès 1 248), image 3:2 pleine tuile, légende au **cartouche de réserve** (A8) sur l'image ; en dessous, rangée de tuiles au-dessus de la grille (4 puis 2 par ligne), légende sous l'image. La vignette de carte reste servie à **274 px, sa taille de conception exacte**, dès 1 248 px de fenêtre — la mesure d'A9 est préservée. **Titre COURT** (celui de la planche, lu par `titreCourt()`), le titre long du frontmatter restant au `<h1>` de la fiche et au référencement. **Cartes strictement homogènes** : aucun rang de statut, puisque le statut n'est plus une information publiée (voir « Localisation et chronologie » ci-dessous). Tout sur une page ; le filtre par secteur réduit la grille, il ne pagine pas. Depuis le 2026-08-27 : **deux navigations de filtre** au-dessus de 1 024 px — le rail visuel à gauche et une barre textuelle de chips en tête de grille, dans une rangée d'en-tête partagée qui aligne par construction la tuile « Tous » et la première ligne de cartes ; les deux jeux de boutons sont synchrones (état par valeur de filtre).
...
| `appui.svg` | `viewBox 0 0 552 368` — **la fiche entre 480 et 879 px** (il a aussi servi le hero de l'accueil jusqu'au 2026-08-26, remplacé par un cliché du corpus secteurs) |
...
| Insécables des `aria_label` (contrôle et correction) | `scripts/insecables-aria-planches.py` — sans argument il mesure, `--appliquer` il écrit ; **ce champ seul**, il n'est jamais dessiné donc aucune géométrie n'en dépend ; lexique d'unités importé de `injection-typographique.py`, jamais recopié |
...
3. **Design system charte v3 « Plans et profondeur » — révision 2.1** (rampe 197°, trois rangs d'ombre, filets par opacité, **huit amendements A1–A8, plus A9–A16 d'application** — A11 à A14 : l'infléchissement motion du 2026-08-27 ; A15 et A16 : le monogramme repris sur demande client le 2026-09-11) — voir `.claude/rules/tailwind-design-tokens.md` (§ Les amendements) et la spec `docs/superpowers/specs/2026-08-06-ft2e-charte-v3-plans-profondeur.md`. La charte prévaut sur tout support existant ; **en cas de contradiction interne à la charte, la mesure prévaut sur la règle**.
...
15. **Aucun honoraire FT2E ne se publie — demande client du 2026-09-16.** Ni montant, ni **taux** de rémunération ou de mission, ni part de groupement, ni le vocabulaire qui y renvoie (« proposition d’honoraires » → **« proposition de mission »**). ⚠ **Un taux appliqué à une enveloppe publiée redonne le montant** : publier l’un interdit l’autre. Restent publiables les chiffres du **maître d’ouvrage** — marchés de travaux, estimations de lots, coût d’objectif : ils démontrent la portée d’une mission sans dire ce qu’elle a rapporté. ⚠⚠ **La portée n’est pas la seule prose : `public/` est recopié dans `dist/`, donc chaque `planche.json` est TÉLÉCHARGEABLE à son URL**, champs non affichés compris. C’est par là que la donnée avait fui — `exclusions_appliquees` recensait les montants écartés **en les citant**, sur **41 dossiers sur 47** contre une quinzaine de fiches en prose. Un champ non affiché n’est pas un champ non publié. Règle, formulation type et contrôle rejouable sur les trois corpus : `.claude/rules/content-collections.md` § « Aucun honoraire FT2E ne se publie ».
...
### Trajectoire de production — arrêtée par l’utilisateur le 2026-09-06

L’ordre est **donné**, il ne se réordonne pas par commodité d’exécution :

1. **Le monogramme** — approche du mot et signature (demande client). ✅ **FAIT le 2026-09-11** (amendements A15 et A16) — trois réserves ouvertes, § 8 du plan.
2. **La finalisation.**
3. **La relecture complète des contenus par le client**, qui remonte ses corrections. *Rien ne se fige avant ce retour.*
4. **Les corrections**, puis un **audit complet et approfondi**.
5. **La bascule et la mise en production sur le serveur OVH.**

⚠ Le tri par « ce qui est exécutable sans attendre personne » reste proscrit — il favorise le polissage et écarte la production. Ici, l’ordre vient du client.

⚠ **L’échéance du 1ᵉʳ janvier 2027 tombe avant la mise en production si le calendrier glisse** : `MILLESIME_LIVRAISON_ANNONCE` fait alors échouer le build sur les quinze affaires sans réception prononcée. À signaler à l’étape 4, et **jamais à corriger en poussant la constante à 2027** — la réponse est d’aller relever les réceptions.
...
| **Chantier du bloc SECTEURS de l'accueil** — **monté le 2026-08-25** : six arbitrages rendus (A10, cote 552 × 345, tirage du film au build, clichés réels © FT2E), coupe déployée en ligne, corpus de 44 clichés versé. Ajusté le 2026-08-26 (position 03, vedette remaniée, légendes de projets sur 27 clichés, cliché principal lié à `/references?secteur=`, filtres en vignettes) ; poursuivi le 2026-08-26 au soir (vedette à deux rangs, rail de filtres de /references, mini-coupe « Tous », taxonomie des secteurs renommée et réordonnée, cliché Aurora au hero — bureau seul, arbitrage LCP) ; ⚠ **les correspondances de clichés sont REÇUES et appliquées le 2026-09-04** (tableau FT2E `correspondance Site projet.odt`, 61 lignes — les 44 clichés publiés y trouvent tous leur projet, zéro clé orpheline) : 17 légendes génériques devenues des noms de projet, plus 3 reprises dont un défaut d'accessibilité (deux clichés d'un même secteur portaient la même légende, donc indistinguables dans la liste lue par un lecteur d'écran) ; deux légendes restent descriptives, et c'est fidèle au document. **Le gel du film n'est plus bloqué mais reste un arbitrage** — le film est tiré au hasard à chaque déploiement. Restent les validations FT2E, le passage NVDA et le NOUVEAU chantier MOTION (infléchissement de charte demandé le 2026-08-27, à arbitrer en A11+) — § 5 à 8 du plan, prompt de suite en annexe D. La maquette est `docs/maquettes/ft2e-accueil-bloc-secteurs.html` — ⚠ page **bundlée**, son contenu vit dans un manifeste JSON, la lire brute ne montre que l'enveloppe | **`docs/superpowers/plans/2026-08-17-chantier-bloc-secteurs.md`** |
...
| **Chantier des 27 NOUVELLES fiches références — CLOS le 2026-09-03 à 47 fiches** (23 → 47, vingt-quatre sessions N01-N24, objectif de 50 revu faute de matière ; le prompt de la session suivante porte LA QUESTION et les points ouverts, pas un dossier : annexe Y. Historique : 23 → 50, 1 session = 1 dossier, pipeline et prompt d'initialisation en annexe A ; les archives sont fournies par l'utilisateur, l'ancien fonds étant supprimé) | **`docs/superpowers/plans/2026-08-27-chantier-27-nouvelles-fiches.md`** |
| **Chantier des SIX ARTICLES SEO de lancement — CLOS le 2026-09-04, 6 sur 6** (c’était le seul critère de sortie de Phase 4 entièrement dans notre camp ; ⚠ le vrai chantier n’était pas « écrire six textes » mais « câbler six textes » — le maillage pilier → article n’existait PAS avant la N26, **8 des 11 piliers desservis contre 0**, et ce plafond est structurel ; ⚠ deux sujets ont été RÉANCRÉS en N27, la mesure ayant montré que les titres de `docs/17` présupposaient une matière absente — 1 fiche géothermique sur 47, 1 fiche nommant Revit ; sujets dits « à valider en cadrage » : les écrire n’est pas les valider ; bilan de clôture au § 5, plafond au § 7, prompt de la N28 en annexe B) | **`docs/superpowers/plans/2026-09-04-chantier-six-articles-seo.md`** |
| **Chantier du MONOGRAMME — CLOS le 2026-09-11, amendements A15 et A16 posés** (approche du mot à 8 unités d’écart, signature « BET FLUIDES / ET THERMIQUE » sur deux lignes justifiées ; ⚠ le dessin amendé n’est plus la transcription du § 07 et **ne doit pas être « corrigé » vers le PDF** ; ⚠ justifier en chasse fixe impose un interlettrage **par ligne**, qui se **recalcule** et ne s’ajuste pas à l’œil ; ⚠ `getBBox()` ne mesure pas l’encre mais l’approche — piège qui aurait fait une signature 4 à 6 unités trop courte ; 🟠 trois réserves : libellé à confirmer, portée non arbitrée (`aria-label` et `SITE_TAGLINE` inchangés), périmètre annoncé rétréci ; bilan au § 8, prompt de la N29 en annexe B) | **`docs/superpowers/plans/2026-09-06-chantier-monogramme.md`** |
