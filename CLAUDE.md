...
- 🔴 **Le monogramme est à reprendre — demande client du 2026-09-06**, et c’est le chantier immédiat. Deux points : resserrer légèrement l’approche du mot F T 2 E, et remplacer la signature « BUREAU D’ÉTUDES TECHNIQUES », jugée trop générique et trop longue. ⚠ **`Logo.astro` est la transcription EXACTE du dessin de référence de la charte** (vérifié contre `branding-v3-bis/FT2E-charte-graphique-rev2.1-source.html`) : les deux demandes **modifient la charte**, elles ne corrigent aucune dérive — d’où les amendements **A15** (approche) et **A16** (signature), à consigner dans `.claude/rules/tailwind-design-tokens.md` **au sein du même commit** que le composant. Mesuré le 2026-09-06 : le mot fait **131,0 unités** d’encre, la signature actuelle **200,2** — soit **× 1,53**, le client a raison et c’est chiffré ; des trois pistes proposées **une seule tient sur une ligne** (« BUREAU FLUIDES »), et l’une d’elles est **plus longue que l’actuelle**. ⚠ Et un point de fond, pas graphique : une signature « fluides » seule contredirait les quatre expertises et sept secteurs que le site présente (électricité, SSI, BIM). Plan, mesures et cinq arbitrages : `docs/superpowers/plans/2026-09-06-chantier-monogramme.md`.
...
1. **Le monogramme** — approche du mot et signature (demande client).
2. **La finalisation.**
...
| **Chantier du MONOGRAMME — OUVERT le 2026-09-06 sur demande client, chantier IMMÉDIAT** (resserrer l’approche du mot F T 2 E, remplacer la signature ; ⚠ `Logo.astro` transcrit EXACTEMENT le dessin de la charte, donc ce sont des **amendements A15 et A16**, à consigner dans `.claude/rules/tailwind-design-tokens.md` au même commit ; mesures faites — mot 131,0, signature actuelle 200,2 soit × 1,53, une seule piste tient sur une ligne ; cinq arbitrages à soumettre en ouverture, dont un de **positionnement** et non de graphisme ; prompt de la N28 en annexe A) | **`docs/superpowers/plans/2026-09-06-chantier-monogramme.md`** |
